...
namespace MO_32_2_Chuprova_BoxBox.NeuroNet
{
    abstract class Layer
    {
        ...
        protected ActivationType activationType; // Тип функции активации
        protected double dropoutProbability; // Вероятность Dropout
        ...
        // Метод для передачи данных с учетом режима обучения (для Dropout)
        public void SetData(double[] data, bool isTraining)
        {
            for (int i = 0; i < numofneurons; i++)
            {
                Neurons[i].Activator(data, isTraining);
            }
        }

        // Конструктор
        protected Layer(int non, int nopn, NeuronType nt, string nm_layer, ActivationType actType = ActivationType.Tanh, double dropoutProb = 0.0)
        {
            ...
            // Настройка момента и скорости обучения
            Settings(actType);
            ...
            for (int i = 0; i < non; i++)
            {
                ...
                Neurons[i] = new Neuron(tmp_weights, nt, activationType, dropoutProbability);
            }

            if (!File.Exists(pathFileWeights) || true)
            {
                WeightsInitialize(MemoryMode.SET, pathFileWeights);
            }
        }

        private void Settings(ActivationType type)
        {
            switch (type)
            {
                case ActivationType.Sigmoid:
                    learningrate = 0.0659d;    
                    momentum = 0.68d;         
                    break;
        ...
        abstract public void Recognize(Network net, Layer nextLayer, bool isTraining = false);
